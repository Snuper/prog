...
{
	return (((left * (right * right - 3)) - (right * (left * left - 3))) / ((right * right - 3) - (left * left - 3)));
}
...
int main (void)
{
	...
	{
		for(int step = 1; mymod2(c_midl(l_interval, r_interval) - c) >= eps; step++)
			{
				cout << endl << step << endl;
