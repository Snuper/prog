...
//				step = 1;
//			}
			y_mas_for_chek[i][du_step - 1 - step] = y_mas[i];
...
//				cout << endl << "Korektirovka: " << fabs(prev_y_mas[i][j] - next_y_mas[i][j * 2]) << " < " << 15 * eps << endl;
				...
				else
				{
					...
					cout << endl << " j = " << j << "  &&  j * 2 + 1 = " << j * 2 + 1 << "  prev_shag = " << prev_shag << " h =  " << h << endl;
					cout << endl << "y(h): " << prev_y_mas[i][j] << endl;
					cout << endl << "y(h/2): " << next_y_mas[i][j * 2 + 1] << endl;
					cout << endl << "Korektirovka: " << fabs(prev_y_mas[i][j] - next_y_mas[i][j * 2 + 1]) << " < " << 15 * eps << endl;
					test_all_mas = false;
					break;
				}
