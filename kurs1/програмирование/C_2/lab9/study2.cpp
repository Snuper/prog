...
typedef struct st
     { char data[20];
       struct st *next;} spis;
spis * create(void); //функци€ создани€ списка
void list(spis *head); // функци€ просмотра списка
void free_spis(spis *head); //функци€ освобождени€ пам€ти
main()
{ system ("CLS");
 ...
 spis * create(void)
{  spis *p, *pred, *h; 
   // pred Ц указатель на предыдущую структуру
  // h- указатель на первую структуру
   h=pred=new spis; //выделение пам€ти дл€ первой структуры
